...
def classify(host, mbfc):
    if not host:
        return dict(
            domain_type="unknown", bias=None, factual_reporting=None, mbfc_matched=False
        )
    if any(host == u or host.endswith("." + u) for u in UGC_DOMAINS):
        return dict(
            domain_type="ugc_platform",
            bias=None,
    ...
    return dict(
        domain_type=dtype, bias=None, factual_reporting=None, mbfc_matched=False
    )
